...
def load_test_data(data_path: str | Path = DATA_PATH) -> dict[str, pd.DataFrame]:
    ...
    base_path = Path(data_path)
    app_usage_df = _load_csv(base_path / "test" / "test_app_usage.csv")
    web_visits_df = _load_csv(base_path / "test" / "test_web_visits.csv")
    claims_df = _load_csv(base_path / "test" / "test_claims.csv")
    churn_labels_df = _load_csv(base_path / "test" / "test_churn_labels.csv")
    ...
    churn_labels_df["signup_date"] = pd.to_datetime(churn_labels_df["signup_date"])

    return {
        "app_usage": app_usage_df,
        "web_visits": web_visits_df,
        "claims": claims_df,
