...
app_sessions_per_member = (
    ...
    .agg(
        ...
        avg_daily_app_sessions=('session_date', lambda x: x.count() / x.nunique() if x.nunique() > 0 else 0)
    )
    .reset_index()
)
...
app_features = app_features.merge(app_sessions_per_member, on='member_id', how='left')
